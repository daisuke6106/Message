...
public class MessageFlyweightTest extends TestCaseTemplate {
	...
	public void getInstance() {
		...
		// getInstanceを実行
		MessageFile messageFile = MessageFlyweight.getInstance("DummyMessage.properties");
		// キャッシュに保持されたインスタンスを取得し、インスタンスが一致するか確認
		Map<Locale, MessageFile> messageFileMap = MessageFlyweight.messageFileMap.get("DummyMessage.properties");
		...
		// キャッシュインスタンスを初期化
		// getInstanceを実行
		messageFile = MessageFlyweight.getInstance("DummyMessage.properties");
		// キャッシュに保持されたインスタンスを取得し、インスタンスが一致するか確認
		messageFileMap = MessageFlyweight.messageFileMap.get("DummyMessage.properties");
		assertThat(new Integer(messageFileMap.size()), is (new Integer(1)));
		assertThat(messageFileMap.get(Locale.getDefault()), is (messageFile));
		...
		// キャッシュインスタンスを初期化
		// getInstanceを実行
		messageFile = MessageFlyweight.getInstance("DummyMessage.properties", Locale.US);
		// キャッシュに保持されたインスタンスを取得し、インスタンスが一致するか確認
		messageFileMap = MessageFlyweight.messageFileMap.get("DummyMessage.properties");
